...
2. **Crear la base de datos.** En `http://localhost/phpmyadmin` → pestaña **Importar** → elegí `database/database.sql`.
   Eso crea la base `sh_servicios` con la configuración inicial.
   *(Opcional: importá también `database/seed.sql` para tener productos de ejemplo.)*
...
├── database/            database.sql (obligatorio) + seed.sql (ejemplo)
